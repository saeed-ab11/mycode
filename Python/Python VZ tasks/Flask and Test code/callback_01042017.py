...
def home():
    ...
    return "The customer callback api"
...
def callbackFirmware():
    ...
    # response = {"status code": "200", "response message": my_test_data[2]}
    response = {}
    ...
    try:
        ...
        if check_keys_result:
            raise Exception("Missing fields: {}".format(str(tuple(check_keys_result))))

        for item in js_data.keys():
            if not js_data.get(item, ""):
                raise Exception("Missing value: {}".format(str(item)))

            if item =="notificationType" and js_data.get(item) != api_name:
        ...
        if second_dict is not None:
            for fw_dict in js_data.get(second_dict):
                ...
                for item in fw_dict.keys():
                    ...
                    if item == "status" and (fw_dict.get(item) != "succeed" and fw_dict.get(item) != "fail"):
                        raise Exception("The value of {} in the device list index {} must be:"
                                        " 'succeed' or 'fail'. ".format(str(item), str(idx)))
        ...
        # Check and raise if have
        if fw_result_agg:
            raise Exception("Missing fields in firmwareList: {}".format(str(tuple(fw_result_agg))))

    except Exception, err_msg:
        code = 400
        response["errorCode"] = "400"
        response["response message"] = str(err_msg)
        # some impl
