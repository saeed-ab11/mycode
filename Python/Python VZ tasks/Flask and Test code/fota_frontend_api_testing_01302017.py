...
def firmware_api(account_name, i, headers):
    api = '/firmware'
    full_api = api + "/" + account_name
    ...
    try:
        ...
        print 'status_code= ' + str(resp.status_code)
        # result_to_file(i, account_name, api, 'GET', 'List all firmwares for account', url, headers, "n/a", resp)
        # response.text
        print resp.json()
        return resp.json()
...
def reports_account_api(account_name, i, headers):
    api = '/reports'
    full_api = api + "/" + account_name + "/account"
    ...
    try:
        ...
        print 'status_code= ' + str(resp.status_code)
        result_to_file(i, account_name, api, 'GET', 'Get account firmware upgrade history', url, headers, "n/a", resp)
        # response.text
        print resp.json()
        return resp.json()
...
def reports_device_api(account_name, deviceId, i, headers):
    api = '/reports'
    full_api = api + "/" + account_name + "/device/" + deviceId
    ...
    try:
        ...
        print 'status_code= ' + str(resp.status_code)
        result_to_file(i, account_name, api, 'GET', 'Get device firmware upgrade history', url, headers, "n/a", resp)
        # response.text
        print resp.json()
        return resp.json()
...
@staticmethod
def get_shifted_date_time_from_now(**kwargs):
    return (datetime.datetime.now() + datetime.timedelta(**kwargs)).strftime("%d-%b-%Y %I:%M:%S %p")


def test_case_1(account_name, participantName, fotaType, licenseCount, i, headers):
    ...
    # startDate_future = get_shifted_date_time_from_now()
    # print 'StartDate_future= ', startDate_future

    startDate_future = "2017-10-24"

    firmwareName_response = firmware_api(account_name, i, headers)

    for firmware in firmwareName_response:

        firmwareName = firmware.get("firmwareName")
        model = firmware.get("model")
        make = firmware.get("make")
        fromVersion = firmware.get("fromVersion")

        device_imei_list_for_upgrade = []

        for device in devices_api_response:
            if device.get("model") == model and device.get("make") == make and device.get("firmware") == fromVersion:
                device_imei_list_for_upgrade.append(device.get("imei"))

        upgrades_schedule_api(device_imei_list_for_upgrade, account_name, startDate_future, firmwareName,
                              participantName, i + 3, headers)

        # if device_imei_list_for_upgrade:
        #     if "TS-hFOTA-Evnt" in fotaType:
        #         upgrades_schedule_api(device_imei_list_for_upgrade, account_name, startDate_future, firmwareName,
        #                           participantName, i + 3, headers)
        #     else:
        #         if len(device_imei_list_for_upgrade) <= licenseCount:
        #             upgrades_schedule_api(device_imei_list_for_upgrade, account_name, startDate_future, firmwareName,
        #                                   participantName, i + 3, headers)
        #         else:
        #             print "license count is not enough for updating"
        #
        # else:
        #     print 'No upgrade is needed - device_imei_list_for_upgrade is empty'

    remove_licenses_api_response = remove_licenses_api(device_imei_list, account_name, i + 4, headers)

    reports_account_api_response = reports_account_api(account_name, i + 5, headers)

    j = 6
    for device_imei in device_imei_list:
        reports_device_api(account_name, device_imei, i + j, headers)
        j += 1


def test_case_2(account_name, participantName, i, headers):
    ...
    # firmwareName_response = firmware_api(account_name, i, headers)
    #
    # firmwareName = firmwareName_response
    #
    # print 'firmwareName= ', firmwareName[0].get("firmwareName")
    #
    # upgrades_schedule_api(device_imei_list, account_name, startDate_future, firmwareName, participantName, i+3, headers)
    #
    # remove_licenses_api_response = remove_licenses_api(device_imei_list, account_name, i + 4, headers)
    pass
...
def run_test_plan():
    ...
    if re.match("2[0-9]{2}", str(response_code)):
        ...
        for subscriber in subscriptions_list:
            test_case_1(subscriber.get("accountName"), subscriber.get("participantName"), subscriber.get("fotaType"),
